...
export default {
    ...
    emits: ["open", "close", "update:modelValue", "input", "clear", "change"],
    model: {
        prop: "modelValue",
        ...
        textShow() {
            ...
            if (this.multiple) {
                ...
                if (Array.isArray(currentValues) && currentValues.length > 0) {
                    const selectedItems = this.mixinDatacomResData.filter((item) =>
    ...
    methods: {
        ...
        getCurrentValues() {
            if (this.multiple) {
                return Array.isArray(this.valueCom)
                    ? this.valueCom
                    : this.valueCom
                      ? [this.valueCom]
                      : [];
        ...
        initDefVal() {
            ...
            if ((this.valueCom || this.valueCom === 0) && !this.isDisabled(this.valueCom)) {
                ...
                if (strogeValue || strogeValue === 0) {
                    ...
                    if (this.defItem > 0 && this.defItem <= this.mixinDatacomResData.length) {
                        defItem = this.multiple
                            ? [this.mixinDatacomResData[this.defItem - 1].value]
                            : this.mixinDatacomResData[this.defItem - 1].value;
                    }
                    defValue = defItem;
                }
                if (
                    defValue ||
                    defValue === 0 ||
                    (this.multiple && Array.isArray(defValue) && defValue.length > 0)
                ) {
                    this.emit(defValue);
                }
            }
            ...
            if (this.multiple) {
                const selectedValues = Array.isArray(defValue)
                    ? defValue
                    : defValue
                      ? [defValue]
                      : [];
        ...
        isDisabled(value) {
            if (Array.isArray(value)) {
                ...
                return value.some((val) => {
                    return this.mixinDatacomResData.some(
                        (item) => item.value === val && item.disable,
                    );
                });
            } else {
        ...
        checkBoxChange(res) {
            ...
            const currentValues =
                range && range.length > 0
                    ...
                    : [];
            const selectedItems = this.mixinDatacomResData.filter((dataItem) =>
                currentValues.includes(dataItem.value),
            );
        ...
        <view class="uni-stat-box" :class="{ 'uni-stat__actived': current }">
            <view
                ...
                :class="{
                    ...
                    'uni-select--wrap': shouldWrap,
                    'border-default': mode == 'default',
                    'border-bottom': mode == 'underline',
                }"
            >
                <view
                ...
                >
                    ...
                    <template v-else>
                        <view
                            ...
                            :class="{ 'uni-select__input-text--wrap': shouldWrap }"
                        >
                            <view class="padding-top-bottom" :class="'align-' + align">{{
                                textShow
                            }}</view>
                        </view>
                        <view
                    ...
                    <view
                        ...
                        @click.stop="clearVal"
                        class="uni-select__icon uni-select__icon--clear"
                    >
                        <view class="i-lucide-x mr-1 text-xs" />
                    </view>
                    <view
                        key="arrow-button"
                        v-else-if="!hideRight"
                        class="uni-select__icon uni-select__icon--arrow"
                    >
                        <view
                            :class="showSelector ? 'i-lucide-chevron-up' : 'i-lucide-chevron-down'"
                        />
                    </view>
                </view>
                        ...
                        <template v-if="slotOption">
                            ...
                            >
                                <slot
                                    ...
                                    :itemSelected="
                                        multiple
                                            ? getCurrentValues().includes(itemData.value)
                                            : getCurrentValues() == itemData.value
                                    "
                                ></slot>
                            </view>
                        </template>
...
.uni-select__icon--arrow {
    width: 14px;
    height: 14px;
    color: #999;
}

.uni-select__icon--arrow view {
    width: 14px;
    height: 14px;
}
</style>
