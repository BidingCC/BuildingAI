...
export function MoveDialog({
  ...
  const batchMoveMutation = useBatchMoveDatasetsDocuments(sourceDatasetId);
  ...
  }, [selectedId, documentIds, batchMoveMutation, onOpenChange, onSuccess]);

  const pending = batchMoveMutation.isPending;

  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-md">
        <DialogHeader>
          <DialogTitle>{documentIds.length > 1 ? "批量移动文档" : "移动文档"}</DialogTitle>
          <DialogDescription>
