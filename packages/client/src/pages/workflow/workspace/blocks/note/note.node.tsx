...
export const NoteNodeComponent: BlockNodeComponent<NoteBlockData> = ({ id, data }) => {
    ...
    >
                ...
                {COLOR_OPTIONS.map((option) => (
                  <button
                    ...
                    className={`h-6 w-6 rounded ${option.bg} border-2 ${
                      data.color === option.value ? "border-gray-800" : "border-gray-300"
      ...
      {isEditing ? (
        ...
        <div className="min-h-[100px] cursor-text text-sm whitespace-pre-wrap">
          {data.content || <span className="text-gray-500">双击编辑笔记...</span>}
        </div>
      )}
    </div>
  );
