...
import type { AuthType, BodyType, HttpBlockData, HttpHeader, QueryParam } from "./http.types";
...
export const HttpPanelComponent: BlockPanelComponent<HttpBlockData> = ({ data, onChange }) => {
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="min-h-[300px]">
        ...
        {activeTab === "basic" && (
          <div className="space-y-3">
                ...
                {HTTP_METHODS.slice(0, 4).map((method) => (
                  <button
                    ...
                    className={`rounded px-3 py-2 text-sm font-semibold ${
                      data.method === method.value ? method.color : "bg-gray-100 text-gray-600"
                    }`}
                  >
                    {method.label}
                  </button>
            ...
            <div>
              ...
              />
              <div className="mt-1 text-xs text-gray-500">支持变量: {"{{variable_name}}"}</div>
            </div>
        ...
        {activeTab === "headers" && (
          <div>
            <label className="mb-2 block text-xs font-medium text-gray-700">HTTP 请求头</label>
            <KeyValueEditor
              items={data.headers}
              onUpdate={(items) => onChange({ headers: items as HttpHeader[] })}
        ...
        {activeTab === "params" && (
          <div>
            <label className="mb-2 block text-xs font-medium text-gray-700">URL 查询参数</label>
            <KeyValueEditor
              items={data.queryParams}
              onUpdate={(items) => onChange({ queryParams: items as QueryParam[] })}
          ...
          <div className="space-y-3">
            ...
            {data.auth.type === "api_key" && (
              <>
                ...
                <div>
                  <label className="text-xs font-medium text-gray-700">Header 名称</label>
                  <input
                    type="text"
        ...
        {activeTab === "advanced" && (
          <div className="space-y-3">
            <div>
              <label className="text-xs font-medium text-gray-700">超时时间 (毫秒)</label>
              <input
                type="number"
            ...
            {data.retries! > 0 && (
              <div>
                <label className="text-xs font-medium text-gray-700">重试延迟 (毫秒)</label>
                <input
                  type="number"
            ...
            <div className="space-y-2">
              ...
              <div className="flex items-center gap-2">
                <input
                  ...
                  onChange={(e) => onChange({ validateStatus: e.target.checked })}
                  className="h-4 w-4"
                />
